...
import ecompilerlab.component.ECompilerResourceSourceListItem;
import ecompilerlab.component.OnlineCompilerTask;
import ecompilerlab.component.ResourceLookUpDialog;
...
//        syntaxMap.put("XML", SyntaxConstants.SYNTAX_STYLE_XML);
  ...
  private void initComponents()
  {
    ...
    final JMenuItem menuLookUpClass = new JMenuItem("Find class");
    ...
    menuLookUpClass.addActionListener(new ActionListener()
    {
      @Override
      public void actionPerformed(ActionEvent e)
      {

        final String className = editor.getSelectedText();
        final ResourceLookUpEntry[] resourceLookUpEntries = OnlineCompilerTask.getInstance()
          .doClassLookUp(className);

        if (resourceLookUpEntries != null && resourceLookUpEntries.length > 0)
        {
          ResourceLookUpDialog dialog = new ResourceLookUpDialog(resourceLookUpEntries);
          dialog.setVisible(true);
          final ECompilerResourceSourceListItem selectedResource = dialog.getSelectedResource();
          ...
          if (selectedResource != null)
          {
            ...
            try
            {
              editor.getDocument().remove(selectionStart, className.length());
              editor.getDocument()
                .insertString(selectionStart, selectedResource.getResource().getFullClassName(), null);
            }
    ...
    editor.addMouseListener(new MouseAdapter()
    {
      ...
      public void mouseClicked(MouseEvent e)
      {
        if (e.getButton() == MouseEvent.BUTTON3)
        {
          ...
          {
            menuLookUpClass.setEnabled(true);
          }
          else
          {
            menuLookUpClass.setEnabled(false);
          }
    ...
    textComponents = new JTextComponent[]{(JTextComponent) categories.getEditor().getEditorComponent(), nameTextField,
    ...
    final JComponent[] components = {(JTextComponent) categories.getEditor().getEditorComponent(), nameTextField,
  ...
  private JPanel buildNorthPanel()
  {
    ...
    JComponent[][] components = {{categoryLabel, categories}, {nameLabel, nameTextField}, {tagsLabel, tagsTextField},
      {syntaxLabel, syntaxBox}};
  ...
  private JPanel buildSouthPanel()
  {
    ...
    JScrollPane scrollPanel = new JScrollPane(commentTextField, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
  ...
  public Snippet getSnippet()
  {
    ...
    return new Snippet(id, category, name, tags, code, comment, syntax, locked,
      (currectSnippetLib == null ? new String[]{} : currectSnippetLib));
  }
